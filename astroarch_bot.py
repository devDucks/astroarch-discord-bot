...
class PackageSelect(ui.Select):
    def __init__(self, packages):
        ...
        super().__init__(
            placeholder="Select package to recompile...",
            options=options,
            max_values=1
        )
    ...
    async def approve(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if interaction.user.id != int(self.owner_id):
            await interaction.followup.send(
                "Only the channel owner can approve.",
                ephemeral=True
            )
            return
...
def trigger_workflow(package: str, arch: str, requester: str) -> bool:
